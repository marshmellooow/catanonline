    ...
    <div className="hand">
      ...
      <div className="hand-dev" data-hand-dev>
        {DEV_ORDER.map((card) => {
          ...
          const title = isVp
            ? 'Verdeckter Siegpunkt (+1)'
            : owned === 0
              ? `${DEV_LABEL[card]} — diese Runde gekauft, nächsten Zug spielbar`
              : DEV_LABEL[card];
          return (
            <button
              key={card}
            ...
            >
              ...
              {pending > 0 && owned > 0 && <span className="dev-new" title={`${pending} neu (nächsten Zug spielbar)`}>+{pending}</span>}
              {owned === 0 && !isVp && <span className="dev-lock" title="Diese Runde gekauft">neu</span>}
            </button>
          );
        })}
      </div>
